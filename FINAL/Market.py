...
def connectToMarket(message, index):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as house_socket:
        connect=False
        while not connect:
            print("!!!! connect to market problem")
            try :
...
if __name__== '__main__':
    ...
    market_process = Process(target=market)
    market_process.start()
    home1 = Process(target=home, args=(1,))
    home2 = Process(target=home, args=(2,))
    home3 = Process(target=home, args=(3,))
    home4 = Process(target=home, args=(4,))
